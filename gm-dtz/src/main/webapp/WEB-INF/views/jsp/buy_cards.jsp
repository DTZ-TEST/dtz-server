    ...
    function initData() {
        ...
        for(var i=0;i<goodsMsgs.length;i++){
            ...
            if (i==1){
                ...
                currentContent=val;
                if(temps.length==5){
                   temp+="<input style='display: none;' type=\"radio\" value=\""+val+"\" name=\"total_fee\" checked>"+temps[1]+"钻(送"+temps[4]+"钻)";
                }else{
    ...
    function save() {
        ...
        if (isOk){
            $.ajax({
                ...
                type: "POST",
                url: "<%=path%>/pay/create",
                data: {t:new Date().getTime(),total_fee:item,gameId:localStorage.getItem('gameId')},
                dataType: "json",
