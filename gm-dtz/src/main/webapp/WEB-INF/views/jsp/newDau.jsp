...
<script>
    ...
    function myQuery() {
        $.ajax({
            ...
            async: true,
            type: "POST",
            url: "<%=path%>/data/statistics/newDau",
            data: {
...
//                JSON.parse(jsonStr);//字符串转json
                if (result.code == 1000) {
                    $("#table_data").html('<tr style="color: #ee6a2a;font-size: 18px;"><td>日期</td><td>DAU</td></tr>');
                    $.each(result.datas, function (index, tempData) {
                        var newRow = "<tr onclick='displayData(this.id)' id='data" + tempData.currentDate + "'><td>" + tempData.currentDate + "</td><td>" + tempData.dau + "</td></tr>";
