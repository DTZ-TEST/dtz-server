...
<table id="con_table" cellpadding="5" style="text-align: center;margin-top: 10px;border-bottom: 1px solid #b5b5b5;width: 100%;padding-bottom: 10px;">
    <tr><td colspan="2" style="text-align: left;font-size: 20px;"><select id="type" class="text-input" style="width: 95%;height: 30px;" onblur="judge(3)"><option value="0" selected="selected">给玩家售钻</option><option value="1" >给代理售钻</option></select></td></tr>
    <tr>
     ...
     <tr>
        <td style="text-align: left"><input id="agencyId" type="text" placeholder="输入代理邀请码" onblur="judge(1)" style="width: 85%;height: 25px;"/></td>
   ...
   function uppage()
    ...
    function myQuery() {
        $.ajax({
            ...
            async:true,
            type: "POST",
            url: "<%=path%>/user/buy/card/info",
            data: {pageNo:page,gameId:localStorage.getItem('gameId'),agencyId:$("#agencyId").val(),playerId:$("#playerId").val(), startDate:$("#start_time").val(),endDate:$("#end_time").val(),type:$("#type").val()},
            dataType: "json",
