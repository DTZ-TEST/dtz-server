...
<table class="header_table" cellpadding="0" cellspacing="0">
    <tr>
        ...
        <td><img src="<%=basePath%>/image/income.png" class="header_img2" style="width: 142px;"/></td>
...
<table id="con_table" cellpadding="5" style="text-align: center;margin-top: 10px;border-bottom: 1px solid #b5b5b5;width: 100%;padding-bottom: 10px;">
   <tr>
        <td style="text-align: left"><input id="agencyId"  type="text" placeholder="代理邀请码" style="width: 85%;height: 25px;"/></td>
   ...
   function uppage()
    ...
    function myQuery(append) {
        ...
        $.ajax({
            ...
            async:true,
            type: "POST",
            url: "<%=path%>/user/cash/log",
            data: {pageNo:page,agencyId:agencyId,gameId:localStorage.getItem('gameId')},
            dataType: "json",
            success: function(result){
                if (result.code==1000){
                    ...
                    if(totalPage <= 1){
                     ...
                     $("#page").html(page+"/"+totalPage);
                     $("#table_data").html('<tr style="color: #ee6a2a;font-size: 20px;"><td>邀请码</td><td>金额</td><td>描述</td><td>操作</td></tr>');
                     $.each(result.datas,function (index,tempData){
                        var newRow="<tr><td>"+tempData.agencyId+"</td><td>"+tempData.money+"</td><td>"+tempData.cashDesc+"</td><td><a onclick=fn("+tempData.id+")><span style='color: blue;font-size: 15px;'>补发</span></a></td></tr>";
