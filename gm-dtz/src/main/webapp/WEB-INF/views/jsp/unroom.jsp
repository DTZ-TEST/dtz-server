...
<table cellpadding="5"
    ...
    <tr>
        <td style="text-align: left"><input id="roomId" type="text" pattern="[0-9]*" placeholder="房间ID" style="width: 85%;height: 25px;"/></td>
        <td><input onclick="loadUserInfo()" type="button" value="查询"/></td>
    </tr>
     <tr>
        <td style="text-align: left" colspan="2"><input id="time" type="text"  placeholder="开房时间" style="width: 85%;height: 25px;" disabled="disabled"/></td>
    </tr>
</table>
...
<script>
    ...
    function save() {
        ...
        var a=confirm("您确认要解散"+roomId+"吗?");
        if (a!=true) return;
        $.ajax({
            timeout: 60000,
            async: true,
            type: "POST",
            url: "<%=path%>/user/unroom",
            data: {
                roomId:roomId
            },
            dataType: "json",
