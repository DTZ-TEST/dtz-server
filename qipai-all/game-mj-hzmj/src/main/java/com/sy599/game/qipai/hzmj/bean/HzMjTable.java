...
public class HzMjTable extends BaseTable {
    ...
    private List<HzMj> leftMajiangs;
    /*** 玩家map */
            ...
            if (selfMo) {
                ...
                for (int seat : seatMap.keySet()) {
                    if (seat != winSeat) {
                        ...
                        int losePoint = huPoint + niaoPoint + piaoPoint;
                ...
                for (int winSeat : winList) {
                    ...
                    int point = huPoint + niaoPoint + piaoPoint;
...
//		}//一炮多响去掉
        ...
        if (!HzMjTool.isHu(huHand, isHu7dui())) {
            return;
        }
        if (moGangHuList.contains(player.getSeat())) {
...
//            return;
        ...
        for (HzMjPlayer seat : seatMap.values()) {
            if (seat.getUserId() != player.getUserId()) {
                List<Integer> list = seat.checkDisMajiang(majiangs.get(0), this.canDianPao());
                if (list.contains(1)) {
...
//            setLastWinSeat(masterseat);
            //随机庄家
            lastWinSeat = new Random().nextInt(getMaxPlayerCount()) + 1;
        }
...
//                }
//            }
            if(readyTime >= HzMjConstants.AUTO_READY_TIME){
                //开了托管的房间，xx秒后自动开始下一局
                for (HzMjPlayer player : seatMap.values()) {
