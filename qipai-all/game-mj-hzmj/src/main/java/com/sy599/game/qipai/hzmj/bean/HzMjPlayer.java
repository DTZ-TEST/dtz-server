...
//	}
    ...
    public List<Integer> checkDisMajiang(HzMj majiang, boolean canCheckHu) {
        ...
        if (canCheckHu && passMajiangVal != majiang.getVal()) {
            ...
            copy.add(majiang);
            HzMjTable table = getPlayingTable(HzMjTable.class);
            boolean hu = HzMjTool.isHu(copy, table.isHu7dui());
            if (hu) {
    ...
    public List<Integer> checkMo(HzMj majiang) {
        ...
        int[] arr = new int[6];
        HzMjTable table = getPlayingTable(HzMjTable.class);
        if (HzMjTool.isHu(handPais, table.isHu7dui())) {
    ...
    public boolean checkAutoPlay(int autoType, boolean forceSend) {
        ...
        if (!auto && table.getIsAutoPlay() == 1) {
            ...
            if (timeOut >= HzMjConstants.AUTO_TIMEOUT) {
                //进入托管状态
                auto = true;
                setAutoPlay(true, false);
                setAutoPlayCheckedTime(HzMjConstants.AUTO_TIMEOUT); //进入过托管就启用防恶意托管
            } else {
                if (sendAutoTime == 0) {
                    sendAutoTime = now;
                ...
                int autoTimeOut = HzMjConstants.AUTO_TIMEOUT;
                if (getAutoPlayCheckedTime() >= HzMjConstants.AUTO_TIMEOUT) {
                    //进入防恶意托管
                    if (timeOut >= HzMjConstants.AUTO_TIMEOUT2) {
                        auto = true;
                        setAutoPlay(true, false);
                    }
                    autoTimeOut = HzMjConstants.AUTO_TIMEOUT2;
                }
