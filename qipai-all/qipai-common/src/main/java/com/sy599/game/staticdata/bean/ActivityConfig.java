	 ...
	 */
		...
		if(configInfo != null) {
			...
			if(configInfo.getType() == 0) {// 永久活动
                ...
                if (isOpen && activityId == activity_group_conquest && player != null) {
                    // 有群主身份的用户才能看到活动
                    List<GroupUser> masterList = null;
                    try {
                        masterList = GroupDao.getInstance().loadGroupUsersByUser(player.getUserId(), 0, 1, 30);
                    } catch (Exception e) {
                        LogUtil.errorLog.error("loadGroupUsersByUser|error|" + e.getMessage(), e);
                    }
