...
public class NewPlayerGiftActivityCmd extends BaseCommand {
    ...
    public void execute(Player player, MessageUnit message) throws Exception {
        ...
        if (requestType == 1) {
            ...
            if (newPlayerGiftRecord.contains(rewardIndex)) {
                //已领取
            ...
            if (newPGLiv < rewardBean.getLiveness()) {
                //条件不足
            ...
            int rewardType = rewardBean.getRewardType();  //红包类型
            ...
            if (rewardType == 1) {
                ...
                rewardBean = rewardBean.getRndReward();
                rewardType = rewardBean.getRewardType();  //红包类型
                redbag = rewardBean.getReward();   //红包金额
                if (rewardType == 1) {
                    //1钻石
                    player.changeCards(redbag, 0, true, -1, CardSourceType.activity_newPlayerGift);
                    player.writeComMessage(WebSocketMsgType.res_code_com, "恭喜您获得：钻石x" + redbag);
                    LogUtil.msgLog.info(player.getName() + "恭喜您获得：钻石x" + redbag);
                } else if (rewardType == 2) {
                    //2现金
                    ...
                    // 随机红包
                    rewardType = 2;
                    int rnd = rewardBean.getRndEnd() - rewardBean.getRndStart();
                    redbag = rewardBean.getRndStart() + new Random().nextInt(rnd + 1);
                    if(redbag > 0) {
                        RedBagManager.getInstance().addRedBagReceiveRecord(new RedBagReceiveRecord(player.getName(), redbag));
                    }
                }
            ...
            userJsonInfo.put("rewardType", rewardType);
            userJsonInfo.put("reward", redbag);
            RedBagInfo redBagInfo = new RedBagInfo(player.getUserId(), rewardType, redbag, new Date(), null, RedBagSourceType.newPlayerGift_redbag);
    ...
    public static void updateBigWinNum(Player player) {
        if (ActivityConfig.isActivityOpen(player, ActivityConfig.activity_new_player_gift)) {
            //新人礼包奖活动开启
            UserActivityRecord userActivityRecord = player.getMyActivity().getUserActivityRecord();
