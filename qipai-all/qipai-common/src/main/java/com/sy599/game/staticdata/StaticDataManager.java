...
//        LogUtil.msgLog.info("loadActivityCsvData finished");
    ...
    private static void loadGoodItems() {
        ...
        List<String[]> list;
        if(TableCheckDao.getInstance().checkTableExists0(DbEnum.LOGIN,"t_base_config")){
            list = BaseConfigDao.getInstance().loadValueList(BaseConfigDao.getInstance().selectAllByType("ExchangeConfig"));
            if (list==null){
                list = readCSV("goods.csv", false);
            }
        }else{
            list = readCSV("goods.csv", false);
        }
        ...
        for (String[] values : list) {
            if (values.length>=9){
                ...
                goodsItem.setId(NumberUtils.toInt(values[0],-1));
                goodsItem.setType(NumberUtils.toInt(values[1],-1));
                goodsItem.setName(values[2]);
                goodsItem.setDesc(values[3]);
                goodsItem.setDiscount(NumberUtils.toInt(values[4],100));
                goodsItem.setAmount(NumberUtils.toInt(values[5],0));
                goodsItem.setCount(NumberUtils.toInt(values[6],0));
                goodsItem.setGive(NumberUtils.toInt(values[7],0));
                goodsItem.setRatio(NumberUtils.toInt(values[8],100));
    ...
    private static String getValue(String[] values, int index) {
        if (index >= values.length) {
            LogUtil.msgLog.error("getValue index > lenght-->" + index + ":" + values.toString());
            return "";
        }
        return values[index];
    }
