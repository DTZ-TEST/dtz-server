...
public final class PayConfigUtil extends ResourceHandler {
    ...
    public static int get(int playType,int totalCount,int maxPlayerCount,int payType,Object ext) {
        ...
        StringBuilder strBuilder=new StringBuilder(36).append("pay_type").append(playType).append("_count")
                .append(totalCount).append("_player").append(maxPlayerCount).append("_pay").append(payType);
        if (ext!=null&&ext.toString().length()>0){
            strBuilder.append("_").append(ext);
        }
