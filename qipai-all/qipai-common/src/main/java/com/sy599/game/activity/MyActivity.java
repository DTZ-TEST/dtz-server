...
import com.sy599.game.util.AwardUtil;
...
public class MyActivity {
	...
	/** 金币场活动**/
	private GoldRoomActivity goldRoomActivity;
	...
	public MyActivity(Player player) {
		...
		this.goldRoomActivity = new GoldRoomActivity();
	}
	...
	public void loadFromDB(String data) {
		if (!StringUtils.isBlank(data)) {
            ...
            String goldRoomActivityStr = json.getString(5);
            goldRoomActivity.parseFromJson(goldRoomActivityStr, player != null ? player.getUserId() : 0);
	...
	public UserActivityRecord getUserActivityRecord() {
		...
		if (!TimeUtil.isSameDay(resetTime, curTime)) {// 每日0点重置玩家活动数据
		    if(!activityRecord.getReceiveGrades().isEmpty()) {
                activityRecord.resetConsumeDaimRecordData();
                activityRecord.resetSmashEggTasks();
            }
			activityRecord.setOldBackGiftPlayOne(false);
	...
	public String toJson() {
		...
		json.putString(5, JacksonUtil.writeValueAsString(goldRoomActivity));
		return json.toString();
	}

    public GoldRoomActivity getGoldRoomActivity() {
        if (goldRoomActivity.refresh(player != null ? player.getUserId() : 0)) {
            player.changeActivity();
            player.saveBaseInfo();
        }
        return goldRoomActivity;
    }
}
