...
public final class TimeUtil {
    ...
    public static boolean isSameWeek(Date a, Date b) {
        ...
        cb.setTime(b);
        cb.setFirstDayOfWeek(Calendar.MONDAY);
        cb.setMinimalDaysInFirstWeek(7);
        if (ca.get(Calendar.WEEK_OF_YEAR) != cb.get(Calendar.WEEK_OF_YEAR)) {
            return false;
        }
    ...
    public static int dayOfWeek() {
        Calendar ca = Calendar.getInstance();
        int dayOffWeek = ca.get(Calendar.DAY_OF_WEEK);
        if (dayOffWeek == 1) {
            return 7;
        } else {
            return dayOffWeek - 1;
    ...
    public static void main(String[] args) throws Exception{
        ...
        Calendar ca = Calendar.getInstance();
        ca.setTime(TimeUtil.getDateByString("2019-11-23 00:00:01","yyyy-MM-dd HH:mm:ss"));
        Date da = ca.getTime();

        Calendar cb = Calendar.getInstance();
        cb.setTime(TimeUtil.getDateByString("2019-11-24 00:00:01","yyyy-MM-dd HH:mm:ss"));
        Date db = cb.getTime();

        boolean sameWeek = isSameWeek(da, db);
        System.out.println("sameWeek = "+sameWeek);

        System.out.println(formatDayIntTime(da));
        System.out.println(formatDayIntTime(db));
        System.out.println(dayOfWeek(da));
        System.out.println(dayOfWeek(db));
    }
