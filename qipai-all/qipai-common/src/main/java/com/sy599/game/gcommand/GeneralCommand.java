...
import com.sy599.game.gcommand.com.ConfigCommand;
...
public class GeneralCommand extends BaseCommand {
    ...
    static {
        ...
        /** 打筒子金币场活动**/
        gcomMap.put(WebSocketMsgType.cs_gold_room_activity, new Command(GoldRoomActivityCmd.class, true));
    ...
    public void execute(ChannelHandlerContext ctx, Player player, MessageUnit message) throws Exception {
        if (message.getMsgType() == WebSocketMsgType.cs_com) {
            ...
            if (player != null && (code == 1 || code == 2 || code == 7 || code == 25 || code == WebSocketMsgType.req_com_group_tables)) {
                // 不是Ping 就记下来
                LogUtil.msgLog.info("common command request message:player=" + player.getClass().getName() + ",userId:" + player.getUserId() + ",tableId:" + player.getPlayingTableId() + ",seat:" + player.getSeat() + " code:" + code + " msgType:" + message.getMsgType()
                        + " List<Integer>:" + req.getParamsList() + " List<String>:" + req.getStrParamsList()
                        + " Class:" + cls.getName()
                );
            }
            ...
            if (command.isWait() && playerIsNotNull) {
                ...
                if (GameUtil.USER_COMMAND_MAP.put(userId, cls.getSimpleName()) == null) {
                    try {
                        ...
                        GameUtil.USER_COMMAND_MAP.remove(userId);
                    }
                }
            } else{
                //心跳消息走的是此处
