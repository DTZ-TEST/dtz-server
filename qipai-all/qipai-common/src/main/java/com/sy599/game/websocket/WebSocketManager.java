...
public class WebSocketManager {
	...
	public static void addWebSocket(MyWebSocket webSocket) {
		...
		if (player != null) {
			MyWebSocket myWebSocket = webSocketMap.get(player.getUserId());
			if (myWebSocket != null && webSocket.getCtx() != myWebSocket.getCtx()) {
				// 账号冲突
				myWebSocket.setLoginError(true);
				myWebSocket.accountConflict(player);
				myWebSocket.close();
				LogUtil.e(player.getUserId() + " 账号冲突");
			}
