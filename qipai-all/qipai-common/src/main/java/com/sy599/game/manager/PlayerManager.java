...
//	public static final Map<Integer,Map<Integer,Class<?>>>
	...
	public Player changePlayer(Player oldPlayer, Class<? extends Player> cl) {
		...
		if (oldPlayer.getPlayingTableId() != 0) {
			// LogUtil.monitor_i("changePlayer tableId-->" +
			// oldPlayer.getPlayingTableId());
		}
		try {
			// 先保存数据
    ...
    public void changeConsumeGold(Player player, int gold, int freeGold, int playType) {
		...
		if (totalGold < 0) {
			...
			if (!player.isRobot())
				LogDao.getInstance().insertGoldConsumeCards(player.getUserId(), player.getRegBindId(), player.getGoldPlayer().getUsedGold(), player.getReginTime(),playType);
		}
    }
