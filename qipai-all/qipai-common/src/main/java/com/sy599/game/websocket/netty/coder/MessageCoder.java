...
public final class MessageCoder {
    ...
    public static MessageUnit decode(byte[] bytes, MessageUnit msgUnit) {
        ...
        if (msgUnit == null) {
            ...
            if (length < 0) {
                LogUtil.errorLog.warn("msg length error, decode fail:bytes length={},msgType={},msgLen={}", len, msgType, length);
                return null;
            }

            if (!Commands.contains(msgType)) {
                LogUtil.errorLog.warn("msgType unknown, decode fail:bytes length={},msgType={}", len, msgType);
                return null;
            }

            if (len > MessageUnit.MAX_PACKAGE_LENGTH || length > MessageUnit.MAX_PACKAGE_LENGTH) {
                LogUtil.errorLog.warn("package length too long, decode fail:bytes length={},msgType={},msgLen={}", len, msgType, length);
                return null;
            }
    ...
    public static List<byte[]> encode(MessageUnit msgUnit) {
        ...
        try {
            if (msgUnit.getMsgType() <= 0) {
                ...
                LOGGER.error("message type unknown:class={}", className);
                return null;
            }

            int contentLength = data != null ? data.length : 0;
            if (contentLength > Short.MAX_VALUE) {
                LOGGER.error("message length too long:length={},type={},class={}", contentLength, msgUnit.getMsgType(), className);
                return null;
            }
