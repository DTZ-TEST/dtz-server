...
import static com.sy599.game.websocket.constant.WebSocketMsgType.res_com_group_table_list;
...
public class GroupTableListCommand extends BaseCommand {
    ...
    public void execute(Player player, MessageUnit message) throws Exception {
        ...
        messageBuilder.builder("code", 0);
        messageBuilder.builder("groupId", groupId);
        messageBuilder.builder("room", groupRoom);
        messageBuilder.builder("pageNo", pageNo);
        messageBuilder.builder("pageSize", pageSize);
        ...
        List<GroupTable> tableList = null;
        //&& "1".equals(ResourcesConfigsUtil.loadServerPropertyValue("load_group_tables_from_redis"))
        if (loadTableMark >= 0) {
            if (Redis.isConnected()&& "1".equals(ResourcesConfigsUtil.loadServerPropertyValue("load_group_tables_from_redis"))) {
            	//groupRoom=1;
            	Map<String, String> groupRooms =   RedisUtil.hgetAll(GroupRoomUtil.loadGroupKey(String.valueOf(groupId), 0));
            	 Set<String> groupRoomKeys = groupRooms.keySet();
            	 List<String> allRoom = new ArrayList<String>();
            	 
            	 for(String groupRoom2 : groupRoomKeys) {
            		 Map<String, String> tableMaps =  RedisUtil.hgetAll(GroupRoomUtil.loadGroupTableKey(String.valueOf(groupId), Integer.valueOf(groupRoom2)));
            		 allRoom.addAll(tableMaps.values());
//            		 if(groupRooms)
            	 }
            	 
            	 if(allRoom.size()>0) {
					Integer sum = allRoom.size(); // 记录总数
					...
					allRoom = allRoom.subList(fromIndex,toIndex);
					
            		 tableList = new ArrayList<>(allRoom.size());
                	 for (String str : allRoom) {
...
//                }
                ...
                if (groupRoom <= 0) {
                    ...
                    if ("1".equals(ResourcesConfigsUtil.loadServerPropertyValue("group_hall_all_tables"))) {
                        //大厅显示所有房间
                        tableList = GroupDao.getInstance().loadGroupTables(groupId, -1, 0, 0, pageNo, pageSize);
                    } else {
                        ...
                        if (configs != null && configs.size() > 0) {
                            ...
                            for (GroupTableConfig con : configs) {
                                List<GroupTable> tmpList = GroupDao.getInstance().loadGroupTables(groupId, con.getGroupId().intValue(), orStarted, andNotFull, pageNo, pSize);
        ...
        if (tableList != null) {
            ...
            if (loadMember) {
                ...
                if (stringBuilder.length() > 0) {
                    List<HashMap<String, Object>> members = GroupDao.getInstance().loadTableUserInfo(stringBuilder.substring(1));
        ...
        player.writeComMessage(res_com_group_table_list, groupId, pageNo, pageSize, messageBuilder.toString());
    }
