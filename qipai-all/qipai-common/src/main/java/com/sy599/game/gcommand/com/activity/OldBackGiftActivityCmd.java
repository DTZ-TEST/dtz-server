...
public class OldBackGiftActivityCmd extends BaseCommand {
    ...
    public void execute(Player player, MessageUnit message) throws Exception {
        ...
        if (requestType == 1) {
            if(!record.isOldBackGiftPlayOne()){
            ...
            if (obgrs.keySet().contains(rewardIndex)) {
                //已领取
            ...
            if (loginCount < oldBackGiftActivityBean.getDayCount() || rewardIndex != index) {
                //条件不足
            ...
            if (!recordBoo) {
                ...
                if (oldBackGiftActivityBean.getRewardType() == 1) {
                    //1钻石
                ...
                } else if (oldBackGiftActivityBean.getRewardType() == 2) {
                    //2现金
        ...
        int i = 0;
        for (OldBackGiftActivityBean bean : rewardDiam) {
            //遍历奖励配置集
            if (loginCount == bean.getDayCount()) {
                //是当天
                if (record.getOldBackGifRecord().keySet().contains(i)) {
                    //已领取
                    dayState = 0;  // 已领取
                } else {
                    dayState = 1;  //未领取
    ...
    public static boolean isOpenActivity(Player player) {
        ...
        if (((logoutTime != null && (System.currentTimeMillis() - logoutTime.getTime() >= unLofinDay * DateUtils.MILLIS_PER_DAY)) && totalCount >= payCount) || lastLoginTime > 0) {
            if ((logoutTime != null && (System.currentTimeMillis() - logoutTime.getTime() >= unLofinDay * DateUtils.MILLIS_PER_DAY)) && totalCount >= payCount) {
                //刚回归
                try {
    ...
    public static void setPlayOne(Player player) {
        try {
            UserActivityRecord record = player.getMyActivity().getUserActivityRecord();
            if (!record.isOldBackGiftPlayOne() && isOpenActivity(player)) {
                record.setOldBackGiftPlayOne(true);
                player.getMyActivity().updateActivityRecord(record);
            }
        } catch (Throwable t) {
            LogUtil.errorLog.error("OldBackGiftActivityCmd|error|", t);
        }
    }
}
