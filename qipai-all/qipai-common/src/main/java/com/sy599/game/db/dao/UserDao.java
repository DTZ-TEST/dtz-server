...
//			}
    ...
    public Map<String, Object> loadGoldRoomActivityInvite(long userId) throws Exception {
        Map<String, Object> param = new HashMap<>();
        param.put("userId", userId);
        Map<String, Object> user = (Map<String, Object>) this.getSqlLoginClient().queryForObject("loadGoldRoomActivityInvite", param);
        return user;
    }

    public int changePlayCount1(long userId, int count) {
        Map<String, Object> param = new HashMap<>();
        param.put("userId", userId);
        param.put("playCount1", count);
        try {
            return getSqlLoginClient().update("changePlayCount1", param);
        } catch (SQLException e) {
            LogUtil.dbLog.error("UserDao|updatePlayCount1|" + userId + "|" + count, e);
        }
        return 0;
    }
}
