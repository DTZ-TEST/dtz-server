...
import com.sy599.game.qipai.ahddz.constant.DdzConstants;
import com.sy599.game.qipai.ahddz.rule.CardType;
import com.sy599.game.qipai.ahddz.tool.CardTool;
import com.sy599.game.qipai.ahddz.tool.CardTypeTool;
...
public class DdzTable extends BaseTable implements AhGame{
...
//                            player.calcWin(1, winPoint);
    ...
    public void saveLog(boolean over, long winId, Object resObject) {
        ...
        if(isGroupRoom()){
            ...
            userGroupLog.setGamename("安化斗地主");
            if(playBureau == 1){
                groupPaylogId = TableLogDao.getInstance().saveGroupPlayLogAh(userGroupLog);
            }else if(playBureau > 1 && groupPaylogId != 0){
                userGroupLog.setId(groupPaylogId);
                TableLogDao.getInstance().updateGroupPlayLogAh(userGroupLog);
            }
        }
        if (!isGoldRoom()) {
            for (DdzPlayer player : playerMap.values()) {
...
//        wrapper.putInt(10, payType);
        ...
        wrapper.putLong(15, groupPaylogId);
        return wrapper;
    }
    ...
    public synchronized void checkDeal(long userId) {
        if (isAllReady()) {
            ...
            if(isGroupRoom()){
                ...
                GroupDao.getInstance().updateGroupTableDealCount(keyId, playedBureau+1);
            }
            setLastActionTime(TimeUtil.currentTimeMillis());
...
//        payType = wrapper.getInt(10, 1);
        ...
        groupPaylogId = wrapper.getLong(15, 0);
    }
...
//				}
    ...
    public void createTable(Player player, int play, int bureauCount, List<Integer> params, List<String> strParams, Object... objects) throws Exception {
        ...
        int payType = StringUtil.getIntValue(params, 9, 2);
        if(payType == 1){
            payType = 2;
        }
