...
public class CsMjPlayer extends Player {
	...
	private List<CsMj> getShowCards(Map<Integer, List<CsMj>> cardMap,boolean connect) {
		if(cardMap==null) {
			return handPais;
		}
		List<CsMj> list =null;
	...
	public CsMjiangHu checkHu(CsMj disMajiang, boolean isbegin) {
		...
		if(table.getJiajianghu()==1) {
			if( seat !=table.getMoMajiangSeat()) {
				...
				CsMj mj  = table.getGangHuMajiang(seat);
...
//		getPlayingTable().changeExtend();
	...
	public boolean canHuXiaoHu2(int action,int val) {
		...
		if (action == CsMjAction.ZHONGTUSIXI && huXiaohu.contains(CsMjAction.DASIXI)&& huXiaohuCards.contains(val)) {
			return false;
		} else if (action == CsMjAction.ZHONGTULIULIUSHUN && huXiaohu.contains(CsMjAction.LIULIUSHUN)&& huXiaohuCards.contains(val)) {
			return false;
		}
		return true;
	}
