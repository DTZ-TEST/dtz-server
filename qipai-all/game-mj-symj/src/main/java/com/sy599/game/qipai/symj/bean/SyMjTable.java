...
public class SyMjTable extends BaseTable {
    ...
    private List<SyMj> leftMajiangs;
    /*** 玩家map */
    ...
    public void calcOver() {
        ...
        ClosingMjInfoRes.Builder res = sendAccountsMsg(playBureau == totalBureau, selfMo, winList, birdMjIds, seatBirds, seatBridMap, catchBirdSeat, false);
        if (!winList.isEmpty()) {
            if (winList.size() > 1) {
                // 一炮多响设置放炮的人为庄家
    ...
    public boolean isAllReady() {
        if (super.isAllReady()) {
            if (getKeChui() == 1) {
                ...
                ComRes.Builder com = SendMsgUtil.buildComRes(WebSocketMsgType.res_code_keChui, getTableStatus());
                for (SyMjPlayer player : seatMap.values()) {
                    if (player.getChui() < 0) {
        ...
        if (player.isAlreadyMoMajiang()) {
            ...
            for (SyMj card : cards) {
                ...
                if (checked.containsKey(card.getVal())) {
                    ...
                    lackPaiList = SyMjTool.getLackList(cardArr, hzCount, true);
                    cardArr[cardIndex] = cardArr[cardIndex] + 1;
                    if (lackPaiList.size() > 0) {
                        checked.put(card.getVal(), lackPaiList);
                    } else {
            ...
            int hzCount = SyMjTool.dropHongzhong(cards).size();
            int[] cardArr = HuUtil.toCardArray(cards);
            List<SyMj> lackPaiList = SyMjTool.getLackList(cardArr, hzCount, true);
            if (lackPaiList == null || lackPaiList.size() == 0) {
                return;
            }
            PlayCardResMsg.TingPaiRes.Builder ting = PlayCardResMsg.TingPaiRes.newBuilder();
