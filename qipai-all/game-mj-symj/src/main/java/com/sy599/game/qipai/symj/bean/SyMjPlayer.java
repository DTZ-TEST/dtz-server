...
//	}
    ...
    public boolean checkAutoPlay(int autoType, boolean forceSend) {
        ...
        if (!auto && table.getIsAutoPlay() == 1) {
            ...
            if (timeOut >= SyMjConstants.AUTO_TIMEOUT) {
                //进入托管状态
                auto = true;
                setAutoPlay(true, false);
                setAutoPlayCheckedTime(SyMjConstants.AUTO_TIMEOUT); //进入过托管就启用防恶意托管
            } else {
                if (sendAutoTime == 0) {
                    sendAutoTime = now;
                ...
                int autoTimeOut = SyMjConstants.AUTO_TIMEOUT;
                if (getAutoPlayCheckedTime() >= SyMjConstants.AUTO_TIMEOUT) {
                    //进入防恶意托管
                    if (timeOut >= SyMjConstants.AUTO_TIMEOUT2) {
                        auto = true;
                        setAutoPlay(true, false);
                    }
                    autoTimeOut = SyMjConstants.AUTO_TIMEOUT2;
                }
