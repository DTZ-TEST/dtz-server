...
//	}
     ...
     */
    public List<Integer> checkMo(ZzMj majiang) {
        List<Integer> list = new ArrayList<>();
        int[] arr = new int[6];
        ZzMjTable table = getPlayingTable(ZzMjTable.class);
    ...
    public boolean checkAutoPlay(int autoType, boolean forceSend) {
        ...
        if (!auto && table.getIsAutoPlay() == 1) {
            ...
            if (timeOut >= ZzMjConstants.AUTO_TIMEOUT + ZzMjConstants.AUTO_CHECK_TIMEOUT) {
                //进入托管状态
                auto = true;
                ...
                int timeSecond = ZzMjConstants.AUTO_TIMEOUT + ZzMjConstants.AUTO_CHECK_TIMEOUT - timeOut;
        ...
        if (auto) {
            if (getAutoPlayTime() == 0L) {
                ...
                if (autoType == 1) {
                    if (timeOut >= ZzMjConstants.AUTO_READY_TIME) {
                        setAutoPlayTime(0);
                        return true;
                    }
                } else if (autoType == 2) {
