...
//        }
        ...
        try {
            ...
            } else if ("self".equals(platform) || "sy".equals(platform) || "syh5".equals(platform)) {
                // 手机官网注册:sy 手机网页官网注册:syh5
                // 渠道号从100开始
    ...
    // String isFocus = "0";
    // if (pf.equals("webyg")) {
    // WebYouGuang sdk = (WebYouGuang) SdkFactory.getLoginInst(pf,
    // getRequest());
    // isFocus = sdk.isFocus();
    // }
    // map.put("code", 0);
    ...
    private Map<String, Object> selfRegister() throws Exception {
        ...
        if (MD5Util.getStringMD5(md5.toString()).equals(sign)) {
            ...
            if (StringUtils.isBlank(username) || StringUtils.isBlank(password) || !checkUserName(username) || (password.length() < 6 || password.length() > 20)) {
                result.put("code", 992);
                result.put("msg", "username or password not allowed");
            } else {
                RegInfo regInfo;
                regInfo = this.userDao.getUser(username, platform);
                if (regInfo != null) {
    ...
    private Map<String, Object> comRegister() throws Exception {
        ...
        // if (StringUtils.isBlank(channel) || !channel.startsWith("A")) {
        // result.put("code", 6);
        // result.put("msg", "channel error");
        // }
...
//        UserGameSite userGameSite = GameSiteDao.getInstance().queryUserGameSite(userInfo.getUserId());
...
//        if (userGameSite != null && userGameSite.getServerId() > 0) {
//            server = SysInfManager.getInstance().getServer(userGameSite.getServerId());
//        } else
        if (shenhe) {
...
//            if(StringUtils.isNotBlank(userInfo.getPf())&&!userInfo.getPf().equals("self")&&server.getPfs().size()>0&&!server.getPfs().contains(userInfo.getPf()) ) {// 如果玩家当前在另外的pf对应的应用的房间中 则切到对应pf所配的服务器
//            	server = SysInfManager.loadServer(userInfo.getPf(), 1);
//            }
        } else {
...
//        user.setConnectHost(StaticDataManager.getGradedChatUrl(server, userInfo.getTotalCount()));
...
//        }
    ...
    public void getVerifyCode1() throws Exception {
        ...
        UserMsgVerify msgVerify = userDao.getMsgVerifyByUid(userId);
        sendMsg(msgVerify,user.getPhoneNum(),userId,null);
    ...
    public void getVerifyCode2() throws Exception {
        ...
        RegInfo user = userDao.getUserByPhoneNum(phoneNum);
        if (user==null){
...
//        UserMsgVerify msgVerify = userDao.getMsgVerifyByUid(userId);
//        if (msgVerify==null){
//            OutputUtil.output(1335, "verifyCode dont exist", getRequest(), getResponse(), false);
//            return;
//        }
//        if(System.currentTimeMillis() - msgVerify.getSendTime().getTime()>600*1000||!verifyCode.equals(msgVerify.getVerifyCode())){
//            //验证码超时或者验证失败
//            OutputUtil.output(1333, "verifyCode err", getRequest(), getResponse(), false);
//            return;
//        }
        Map<String, Object> map = new HashMap<>();
        switch (codeType){
...
//     * 忘记密码处的验证短信验证码
//     */
//    public void verifyCode1() throws Exception{
//        String verifyCode= getRequest().getParameter("verifyCode");
//        UserMsgVerify msgVerify = userDao.getMsgVerifyByIp(getIpAddr(getRequest()));
//        if (msgVerify==null){
//            OutputUtil.output(1335, "verifyCode dont exist", getRequest(), getResponse(), false);
//            return;
//        }
//        if(System.currentTimeMillis() - msgVerify.getSendTime().getTime()>600*1000||!verifyCode.equals(msgVerify.getVerifyCode())){
//            //验证码超时或者验证失败
//            OutputUtil.output(1333, "verifyCode out time or err", getRequest(), getResponse(), false);
//            return;
//        }
//        userDao.updateSmsFlag(msgVerify.getUserId(),1);
//        OutputUtil.output(0, "验证成功", getRequest(), getResponse(), false);
//    }
    ...
    public void uploadPassword() throws Exception{
        ...
        if (msgVerify==null||msgVerify.getIsUse()==0){
            OutputUtil.output(1334, "verifyFlag err or outTime", getRequest(), getResponse(), false);
        }
    ...
    private void  sendMsg(UserMsgVerify msgVerify,String phoneNum,long userId,String ip) throws Exception{
        ...
        //至少过一分钟才能发送第二份短信验证
        String verifyCode=userDao.updateUserMsgVerify(userId,phoneNum,ip);
        Properties properties = InitData.keyValueProperties;
        SMSUtil.sendSMSNew(properties.getProperty("sms_appId"), properties.getProperty("sms_appKey"), phoneNum,
                ResourcesConfigsUtil.loadServerPropertyValue("smsConfig","快乐玩游戏"),Integer.parseInt(verifyCode),28681);
        System.out.println("获取短信验证成功："+verifyCode);
    }
...
//        RegInfo user;
    ...
    /*  public void test() throws Exception {
          ...
          for (int i = 0; i < 20000; i++) {
              int index = userDao.getLottyResult(101006);
              if (index == 3) m++;
              if (index == 7) n++;
