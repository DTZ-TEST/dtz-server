...
//			task.isRun(hour, 0);
		...
		// 清理代开房间
		 ...
		 try{
		 	if (SqlDao.getInstance().checkExistsGroupTable()>0){
				int count=0;
				...
				while ((list= GroupDaoManager.getInstance().loadGroupTables(startDate,endDate,pageNo,pageSize))!=null&&list.size()>0){
					Map<String,Integer> userMap=new HashMap<>();
					Map<String,Integer> tableMap=new HashMap<>();
					for (HashMap<String,Object> map:list){
						String key = String.valueOf(map.get("keyId"));
						if (GroupDaoManager.getInstance().updateGroupTableByKeyId(key)>0){
