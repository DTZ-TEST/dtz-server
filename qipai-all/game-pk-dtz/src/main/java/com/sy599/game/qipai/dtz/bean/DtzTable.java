...
import com.sy599.game.qipai.dtz.tool.*;
...
//                    }
            ...
            if (isGoldRoom()){
                if (playedBureau==0||lastWinSeat<=0) {
                    ...
                    try {
                        List<HashMap<String, Object>> list1 = GoldRoomDao.getInstance().loadRoomUsersLastResult(playerMap.keySet(), id);
                        if (list1 != null) {
...
//            LogUtil.msg("set nowDisCardIds: "+ nowDisCardIds+","+id);
    ...
    public void createTable(Player player, int play, int bureauCount, Object... objects) throws Exception {
        ...
        String autoTimeOutDtz;
        if (isMatchRoom()){
            autoTimeOutDtz = ResourcesConfigsUtil.loadServerPropertyValue("autoTimeOutDtzMatch","30,3,8");
        }else{
            autoTimeOutDtz = ResourcesConfigsUtil.loadServerPropertyValue(isGoldRoom()?"autoTimeOutDtzGold":"autoTimeOutDtz","");
        }
        String[] strs = autoTimeOutDtz.split(",");
        dtz_auto_timeout = StringUtil.getIntValue(strs, 0) * 1000;
        dtz_auto_play_time = StringUtil.getIntValue(strs, 1) * 1000;
        dtz_auto_startnext = StringUtil.getIntValue(strs, 2) * 1000;
        dtz_auto_timeout2 = strs.length >= 4 ? StringUtil.getIntValue(strs, 3) * 1000 : dtz_auto_timeout;
    }
...
//        	canSett = loadCanSett(wrapper.getString("canSett"));
    ...
    public ClosingInfoRes.Builder sendAccountsMsg(boolean over, Player winPlayer, boolean isBreak) {
        ...
        Map<Integer, Integer> seatFen = new HashMap<Integer, Integer>();
        int winGroup = isBreak ? -1 : calhasWinGroup();
        //if(!isFirstDissScore(isBreak)){
        for (DtzPlayer player : seatMap.values()) {
        ...
        if (isThreePlayer()) {
            ...
            if (!isGold) {
                totalA = CardTool.calcScore(totalA);
                totalB = CardTool.calcScore(totalB);
                totalC = CardTool.calcScore(totalC);
            }
        } else {
            if (this.jiangli > 0 && winGroup > 0) {
                if (seatFen.containsKey(winGroup)) {
        ...
        MatchBean matchBean = isMatch ? JjsUtil.loadMatch(matchId) : null;
        if (isMatch && matchBean == null) {
            isMatch = false;
        }

        Map<Integer, Integer> winLossPointMap = new HashMap<>();
        if (isMatch) {
            goldPay = 0;
            modeId = "match" + matchId;
...
//                    winLossPoint*=goldRatio;

                    winLossPointMap.put(player.getSeat(), winLossPoint);
                }
            }
        } else if (isGold) {
            try {
                ...
                if (goldRoom != null) {
                    ...
                    int lossTotalPoint0 = 0;
                    int lossTotalPoint1 = 0;
                    for (DtzPlayer player : seatMap.values()) {
                        if (isThreePlayer()) {
                            int winLossPoint = 0;
                            ...
                            winLossPoint *= goldRatio;
                            if (winLossPoint < 0) {
                                lossTotalPoint1 -= winLossPoint;
                                int temp = winLossPoint;
                                if (player.getGoldPlayer().getAllGold() < goldPay - temp) {
                                    temp = (player.getGoldPlayer().getAllGold() > goldPay) ? (-(int) (player.getGoldPlayer().getAllGold() - goldPay)) : 0;
                                }
                                winLossPoint = temp;
                                lossTotalPoint0 -= winLossPoint;
                            }
                            winLossPointMap.put(player.getSeat(), winLossPoint);
                        }
                    }

                    if (lossTotalPoint1 > 0 && lossTotalPoint1 != lossTotalPoint0) {
                        Iterator<Map.Entry<Integer, Integer>> it = winLossPointMap.entrySet().iterator();
                        int lossTotalPoint2 = 0;
                        int win1 = 0, win2 = 0;
                        while (it.hasNext()) {
                            Map.Entry<Integer, Integer> kv = it.next();
                            if (kv.getValue().intValue() > 0) {
                                if (win1 == 0) {
                                    win1 = kv.getKey().intValue();
                                } else {
                                    win2 = kv.getKey().intValue();
                                }
                                int tempPoint = (int) ((kv.getValue().intValue()) * (lossTotalPoint0 * 1.0D / lossTotalPoint1));
                                lossTotalPoint2 += tempPoint;
                                winLossPointMap.put(kv.getKey(), tempPoint);
                            }
                        }
                        if (lossTotalPoint2 < lossTotalPoint0) {
                            if (win2 == 0) {
                                if (win1 > 0) {
                                    winLossPointMap.put(win1, winLossPointMap.getOrDefault(win1, 0) + (lossTotalPoint0 - lossTotalPoint2));
                                }
                            } else if (win1 > 0) {
                                int t = (lossTotalPoint0 - lossTotalPoint2) / 2;
                                winLossPointMap.put(win1, winLossPointMap.getOrDefault(win1, 0) + t);
                                t = lossTotalPoint0 - lossTotalPoint2 - t;
                                winLossPointMap.put(win2, winLossPointMap.getOrDefault(win2, 0) + t);
                            }
                        }
                    }
                    LogUtil.msgLog.info("calc score totalA=" + totalA + ",totalB=" + totalB + ",totalC=" + totalC + ",winLossPointMap=" + winLossPointMap);
                }
            } catch (Exception e) {
            }
        }

        Map matchDataMap = new HashMap();

        if (isMatch) {
            for (DtzPlayer player : seatMap.values()) {
                int tempScore = winLossPointMap.getOrDefault(player.getSeat(), 0);
                matchDataMap.put(player.getUserId(), tempScore);
...
//            }
            ...
            if (scoreMap != null) {
//				build.setTotalFive((scoreMap.get(ScoreType.POINT_5) == null) ? 0 : scoreMap.get(ScoreType.POINT_5)); // 设置一下总5的个数  1
                build.addExt(((scoreMap.get(ScoreType.POINT_5) == null) ? 0 : scoreMap.get(ScoreType.POINT_5)) + "");

//				build.setTotalTen((scoreMap.get(ScoreType.POITN_10) == null) ? 0 : scoreMap.get(ScoreType.POITN_10)); // 设置一下总10的个数  2
                build.addExt(((scoreMap.get(ScoreType.POITN_10) == null) ? 0 : scoreMap.get(ScoreType.POITN_10)) + "");

//				build.setTotalK((scoreMap.get(ScoreType.POINT_K) == null) ? 0 : scoreMap.get(ScoreType.POINT_K)); // 设置一下总k的数个 3
                build.addExt(((scoreMap.get(ScoreType.POINT_K) == null) ? 0 : scoreMap.get(ScoreType.POINT_K)) + "");

//				build.setTotalTongzi(tongziNumByPlayer(player)); // 设置一下筒子的数量 4
                build.addExt((tongziNumByPlayer(player)) + "");
            } else {
//				build.setTotalFive(0); // 设置一下总5的个数 1
                build.addExt(0 + "");
//				build.setTotalTen(0); // 设置一下总10的个数 2
                build.addExt(0 + "");
//				build.setTotalK(0); // 设置一下总k的数个 3
                build.addExt(0 + "");
//				build.setTotalTongzi(0); // 设置一下筒子的数量 4
                build.addExt(0 + "");
            }
            int addPoint = 0;
            if (addScore.size() != 0) {
            ...
            //int totalPoint = isFirstDissScore(isBreak) ? 0 : (player.getDtzTotalPoint() + player.getPoint() + addPoint + player.getRoundScore()), winLossPoint = 0;
            int totalPoint = player.getDtzTotalPoint() + player.getPoint() + addPoint + player.getRoundScore(), winLossPoint = 0;
            player.setDtzTotalPoint(totalPoint);

            int ticketCount = 0;
            if (isThreePlayer()) {
                if (isMatch) {
                    winLossPoint = (int) (winLossPointMap.getOrDefault(player.getSeat(), 0) * matchRatio);
                } else if (isGold) {
                    winLossPoint = winLossPointMap.getOrDefault(player.getSeat(), 0);
                    if (winLossPoint <= 0 || winLossPoint <= goldPay) {
                        player.changeGold((winLossPoint - goldPay), playType);
                        player.updateGoldRoomActivity(false);
                    } else {
                        player.changeGold((winLossPoint - goldPay), 0, playType);
                    ...
                    if (winLossPoint > 0) {
                        ...
                        if (tmpConfig != null && tmpConfig.intValue() > 0) {
                            ticketCount = winLossPoint / (tmpConfig.intValue());
                            if (ticketCount > 0) {
...
//            build.setTotalPoint(isGoldRoom()?(int)player.loadAllGolds():player.getDtzTotalPoint());
            //
            build.setPoint(player.getPoint() + addPoint); //设置一下这个玩家的总分 小结算的

//            if(isBreak){//提前解散
//            	Map<ScoreType, Integer> tempTongXi = tempScoreList.get(player);
//            	if(tempTongXi != null && !tempTongXi.isEmpty()){
//            		build.addExt(tempTongXi.containsKey(ScoreType.POINT_XI)?tempTongXi.get(ScoreType.POINT_XI)+"":0+"");
//            		build.addExt(tempTongXi.containsKey(ScoreType.POINT_JACKER_S)?tempTongXi.get(ScoreType.POINT_JACKER_S)+"":0+"");
//            		build.addExt(tempTongXi.containsKey(ScoreType.POINT_JACKER_B)?tempTongXi.get(ScoreType.POINT_JACKER_B)+"":0+"");
//            		build.addExt(tempTongXi.containsKey(ScoreType.POINT_BD)?tempTongXi.get(ScoreType.POINT_BD)+"":0+"");
//            	}else{
//            		build.addExt(0 + "");
//                    build.addExt(0 + "");
//                    build.addExt(0 + "");
...
//            	Map<ScoreType, Integer> tempTongXi = tempScoreList.get(player);
//            	if(tempTongXi != null && !tempTongXi.isEmpty()){
//            		build.addExt(tempTongXi.containsKey(ScoreType.POINT_TZ_A)?tempTongXi.get(ScoreType.POINT_TZ_A)+"":0+"");
//            		build.addExt(tempTongXi.containsKey(ScoreType.POINT_TZ_K)?tempTongXi.get(ScoreType.POINT_TZ_K)+"":0+"");
//            		build.addExt(tempTongXi.containsKey(ScoreType.POINT_TZ_2)?tempTongXi.get(ScoreType.POINT_TZ_2)+"":0+"");
//            	}else{
//            		build.addExt(0 + "");
//                    build.addExt(0 + "");
//                    build.addExt(0 + "");
...
//            }
            ...
            //getRoundScore()
            //build.addExt((comeOtherTzOrXiScore(player) + (build.getPoint())) + "");
            build.addExt((player.getRoundScore() + (build.getPoint())) + ""); //16
            LogUtil.msg("player:  " + player.getName() + "point --> " + build.getPoint() + " addScore --> " + player.getRoundScore());

//            if(isBreak){
//            	Map<ScoreType, Integer> tempTongXi = tempScoreList.get(player);
//            	if(tempTongXi != null && !tempTongXi.isEmpty()){
//            		build.addExt(tempTongXi.containsKey(ScoreType.POINT_TZ_S)?tempTongXi.get(ScoreType.POINT_TZ_S)+"":0+"");
//            		build.addExt(tempTongXi.containsKey(ScoreType.POINT_TZ_B)?tempTongXi.get(ScoreType.POINT_TZ_B)+"":0+"");
//            	}else{
//            		build.addExt(0 + "");
//                    build.addExt(0 + "");
//            	}
...
//            }
            //19
            build.addExt(String.valueOf(isMatch?matchBean.loadUserScore(player.getUserId()):(isGold?player.loadAllGolds():0)));
        ...
        res.addAllExt(buildAccountsExt(winGroup));
    ...
    public List<String> buildAccountsExt(int winGroup) {
            ...
            synchronized (this) {
                ...
                if (getState() == table_state.play || (getState() == table_state.over && !jiesuan)) {
                        ...
                        if (isGoldRoom()){
                            if(isMatchRoom()){
                                if ("self".equals(player.getPf())&&player.getName().equals("test"+player.getUserId())){
                                    ...
                                    dtz_auto_timeout0 = dtz_auto_timeout + 5000;
                    ...
                    for (DtzPlayer player : seatMap.values()) {
                        ...
                        long  dtz_auto_startnext0;
                        if (isGoldRoom()){
                            if(isMatchRoom()){
                                if ("self".equals(player.getPf())&&player.getName().equals("test"+player.getUserId())){
                                    dtz_auto_startnext0 = 2000;
                                }else{
                                    dtz_auto_startnext0 = dtz_auto_startnext;
                                }
                            }else{
                                dtz_auto_startnext0 = dtz_auto_startnext;
                            }
                        ...
                        if (time - lastActionTime > dtz_auto_startnext0) {
                            LogUtil.msg("房间号=" + getId() + ",玩家=" + player.getUserId() + "[" + player.getName() + "]座位=" + player.getSeat() + "自动托管进入下一局");
                            ...
                            if (player.isStartNextReady()) {
                                this.ready(player);
                                for (Player tableplayer : this.getSeatMap().values()) {
                                    if (tableplayer.getUserId() == player.getUserId()) {
                                        continue;
                                    }
                                    tableplayer.writeSocket(msg1);
                                }
                                for (Player player0 : this.getRoomPlayerMap().values()) {
                                    player0.writeSocket(msg1);
                                }
                            }

                            this.ready();
                            this.checkDeal(player.getUserId());
                            // 如果是最后一个点继续的人需要告诉前台
                            ...
                            TableRes.CreateTableRes.Builder res = this.buildCreateTableRes(player.getUserId(), true, isLastStart).toBuilder();
                            res.setFromOverPop(1);
                            player.writeSocket(res.build());

                            for (Player player0 : this.getRoomPlayerMap().values()) {
                                TableRes.CreateTableRes.Builder res0 = this.buildCreateTableRes(player0.getUserId(), true, false).toBuilder();
    ...
    public boolean createSimpleTable(Player player, int play, int bureauCount, List<Integer> params, List<String> strParams, boolean saveDb) throws Exception {
        ...
        bombScore.load(getWangTongZi());
        changeExtend();
//        DtzSendLog.sendCardLog(id, player.getUserId(), player.getName(), "创建房间", "组分数：" + default_Sore_Dtz);
    ...
    public void createTable(Player player, int play, int bureauCount, List<Integer> params, List<String> strParams, Object... objects) throws Exception {
        ...
        this.creditMode = StringUtil.getIntValue(params, 14, 0);
        this.creditJoinLimit = StringUtil.getIntValue(params, 15, 0);
        this.creditDissLimit = StringUtil.getIntValue(params, 16, 0);
        this.creditDifen= StringUtil.getIntValue(params, 17, 0);
        this.creditCommission = StringUtil.getIntValue(params, 18, 0);
        this.creditCommissionMode1 = StringUtil.getIntValue(params, 19, 1);
        this.creditCommissionMode2 = StringUtil.getIntValue(params, 20, 1);
        this.creditCommissionLimit = StringUtil.getIntValue(params, 21, 100);
        ...
        createTable(player, play, bureauCount, haveJacker, playerCount, showCardNumber);
        /*
         * 600分 AA 8 房主 30
...
//        addScoreBuGroup(2, (map.containsKey(2) ? map.get(2) : 0));
    ...
    private int hasWinGroup() {
        ...
        for (Entry<Integer, Integer> entry : this.groupScore.entrySet()) {
            if ((isMatch&&playedBureau>=totalBureau)|| ((!isMatch)&&isGold && playedBureau>=1)||(!isMatch) && (!isGold)&& playedBureau >= totalBureau || ((!isMatch)&&(!isGold)&&entry.getValue() >= score_max)) {
                return entry.getKey();
            }
        }
        return 0;
    ...
    private int calhasWinGroup() {
        ...
        for (Entry<Integer, Integer> entry : groupScore.entrySet()) {
            if ((isMatch&&playedBureau>=totalBureau)|| ((!isMatch)&&isGold && playedBureau>=1)||(!isMatch) && (!isGold)&& playedBureau >= totalBureau || ((!isMatch)&&(!isGold)&&entry.getValue() >= score_max)) {    //有达到  结算分数的
                if(isThreePlayer()){
...
//        LogUtil.msg("table:" + id + " 结算 时的分数:  1 -> " + groupScore.get(1) + " 2 ->" + groupScore.get(2) + " 没有哪一组达到大结算分数.");
    ...
    public void recordCardAll(List<List<Integer>> cards, DtzPlayer pdkplayer) {
        ...
        for (Entry<DtzPlayer, HashMap<ScoreType, Integer>> entry : bureau.entrySet()) {
            log.append("玩家:").append(entry.getKey().getName()).append(" 现在所有的有分的牌 ->");
            log.append(entry.getValue()).append(" \n");
        }
        LogUtil.msg(log.toString());
    }
...
//		   addSerialScoreToGroup(groupScore());
    ...
    public  boolean isCanJoin0(Player player) {
        ...
        if (consumeCards()) {
            if (payType == 1) {
                //AA制
                needCard = PayConfigUtil.get(playType, 30, calcPlayerCount(getMaxPlayerCount()), 0, getScore_max());
            }
        }
        // 如果玩家的钻石小于玩一局需要的钻石，则返回
        if (checkPay&&(needCard < 0 || needCard>0&&player.getFreeCards() + player.getCards() < needCard) && !GameConfigUtil.freeGame(playType,player.getUserId())) {
            player.writeErrMsg(LangMsg.code_diamond_err);
            return false;
        }

//        if (isOpenGps==1) {
//            if (StringUtils.isBlank(player.getMyExtend().getLatitudeLongitude())||"error".equals(player.getMyExtend().getLatitudeLongitude())) {
//                player.writeErrMsg(LangMsg.code_201);
//                LogUtil.msg("userId:"+player.getUserId()+",tableId:"+getId()+",msg:"+LangHelp.getMsg(LangMsg.code_201));
//                return false;
//            }
//            for (Player player1 : playerMap.values()) {
//                for (Player player2 : playerMap.values()) {
//                    double dis = GameUtil.getDistance(player1.getMyExtend().getLatitudeLongitude(), player2.getMyExtend().getLatitudeLongitude());
//                    if (dis < 100) {
//                        player.writeErrMsg(LangMsg.code_202);
//                        LogUtil.msg("test dis:userId:"+player.getUserId()+",tableId:"+getId()+",msg:"+LangHelp.getMsg(LangMsg.code_202));
//                        return false;
//                    }
//                }
//            }
...
//    					LogUtil.msg("玩家:" + winPlayer + " score : " + winPlayer.getPoint() + " - " + winPlayer.getRoundScore());
    ...
    public void playCommand(DtzPlayer player, List<Integer> cards, CardTypeDtz cardType){
        synchronized (this) {
            ...
            params.append("玩家出牌为：");params.append(JacksonUtil.writeValueAsString(cards));
            params.append(",玩家位置：").append(player.getSeat());
            params.append(",牌局结算状态：").append(this.jiesuan);
            params.append(",当前小局 : ").append(this.dtzRound);
            params.append(",是否不是当局第一人出不要 : ").append(this.masterFP);
            ...
            params.append(",上家出牌为：");params.append(JacksonUtil.writeValueAsString(disCardIds));
...
//    	}
    ...
    public int loadPayConfig(int payType) {
        ...
        switch (payType){
            case 1:
                ret=PayConfigUtil.get(playType, totalBureau, getMaxPlayerCount(), 0, getScore_max());
                break;
            case 2:
                ret=PayConfigUtil.get(playType, totalBureau, getMaxPlayerCount(), 1, getScore_max());
                break;
            case 3:
                ret=PayConfigUtil.get(playType, totalBureau, getMaxPlayerCount(), 3, getScore_max());
                break;
            default:
