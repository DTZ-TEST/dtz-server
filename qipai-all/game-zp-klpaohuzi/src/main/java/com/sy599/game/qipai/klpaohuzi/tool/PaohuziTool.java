package com.sy599.game.qipai.klpaohuzi.tool;

import com.alibaba.fastjson.JSON;
import com.sy599.game.GameServerConfig;
import com.sy599.game.qipai.klpaohuzi.bean.PaohuziHandCard;
import com.sy599.game.qipai.klpaohuzi.bean.PaohuziHuBean;
import com.sy599.game.qipai.klpaohuzi.bean.PaohzDisAction;
...
//				lack.addPhzHuCards(PaohzDisAction.action_zai, toPhzCardIds(cards), huxi);
     ...
     * @return
     */
    public static List<Integer> findIdByVal(List<PaohzCard> copy, int val) {
    ...
    public static PaohzCard autoDisCard(List<PaohzCard> handPais) {
        List<PaohzCard> copy = new ArrayList<>(handPais);
        PaohzCardIndexArr valArr = PaohuziTool.getMax(copy);
        PaohuziIndex index1 = valArr.getPaohzCardIndex(0);// 单牌
        PaohuziIndex index2 = valArr.getPaohzCardIndex(1);// 双牌
        int val = 0;
        if (index1 != null && !index1.getValList().isEmpty()) {
            //第一步找单牌
            val = Collections.min(index1.getValList());
        } else if (index2 != null && !index2.getValList().isEmpty()) {
            //没有单牌，找双牌
            val = Collections.min(index2.getValList());
        }
        if (val == 0) {
            return null;
        } else {
    ...
    public static void main(String[] args) {
        ...
        int outCardHuXi = 0 ;
        List<Integer> valList = new ArrayList<>(Arrays.asList(101,102,103,4,5,6,7,8,9,107,107,108,109,110,110,10,10,107));
        valList = new ArrayList<>(Arrays.asList(101,1,1,2,3,4,101,102,103,105,5,5,6,7,8,102,107,110,104,104,104));
        valList = new ArrayList<>(Arrays.asList(101,101,102,103,104));needDui=true;
        valList = new ArrayList<>(Arrays.asList(1,1,1,4,4,4,7,7,7,10,10,101,102,103,106,107,108,109,110,110,105));needDui=false;
        valList = new ArrayList<>(Arrays.asList(101,101,101));needDui=false;
        int count = 1;
        List<PaohzCard> cardList = val2Card(valList);

        PaohzCard card1 = cardList.get(cardList.size() - 1);
        PaohuziHandCard bean1 = getPaohuziHandCardBean(cardList);
        PaohuziHuLack hu1 = null;
        long start1 = System.currentTimeMillis();
        for(int i = 0 ; i < count ; i++) {
            hu1 = isHu(bean1, card1, isSelfMo, outCardHuXi, needDui, isPaoHu);
        }
        System.out.println("count = " + count + " timeUse = " + (System.currentTimeMillis() - start1) + " ms");
        System.out.println((hu1.isHu() ? "胡啦" : "不胡") + "|" + JSON.toJSONString(hu1));

        PaohuziHandCard bean2 = getPaohuziHandCardBean(cardList);
        PaohzCard card2 = null;
        PaohuziHuLack hu2 = null;
        long start2 = System.currentTimeMillis();
        for(int i = 0 ; i < count ; i++){
            hu2 = isHuNew(bean2, card2, isSelfMo, outCardHuXi, needDui, isPaoHu);
        }
        System.out.println("count = " + count + " timeUse = " + (System.currentTimeMillis() - start2) + " ms");
