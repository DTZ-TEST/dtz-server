...
import com.sy599.game.qipai.klpaohuzi.command.KlPaohuziCommandProcessor;
import com.sy599.game.qipai.klpaohuzi.constant.EnumHelper;
...
public class KlPaohuziPlayer extends Player {
	// 座位id
	private int seat;
	// 状态
...
//        }
	...
	public PaohuziHuLack checkPaoHu(PaohzCard card, boolean isSelfMo, boolean canDiHu) {
		...
		if (card != null) {
			KlPaohuziTable table = getPlayingTable(KlPaohuziTable.class);
	...
	public PaohuziCheckCardBean checkCard(PaohzCard card, boolean isSelfMo, boolean isPassHu, boolean isBegin, boolean isFirstCard, boolean isPass) {
		...
		if (card != null) {
			...
			if (isCanChiPeng && !check.isTi() && !check.isPao() && isCheckChi && !isPass) {
				...
				KlPaohuziPlayer frontPlayer = (KlPaohuziPlayer) table.getPlayerBySeat(table.calcFrontSeat(seat));
	...
	public ClosingPhzPlayerInfoRes.Builder bulidOneClosingPlayerInfoRes(boolean isBoPi, boolean isOver, int bopiPoint) {
        ...
        for(int action : actionTotalArr){
            res.addStrExt(action+"");
        }
		return res;
	}
	...
	public PlayerInTableRes.Builder buildPlayInTableInfo(long lookUid, boolean isrecover) {
		...
		if (seat==table.getShuXingSeat()){
			KlPaohuziPlayer paohuziPlayer = (KlPaohuziPlayer)table.getSeatMap().get(table.getLastWinSeat());
        ...
        int tuoGuanCountDown = PaohuziConstant.AUTO_TIME;
        if (getLastCheckTime() > 0) {
            tuoGuanCountDown = PaohuziConstant.AUTO_TIME - (int)((now - getLastCheckTime())/1000);
            if(tuoGuanCountDown < 0){
                tuoGuanCountDown = 0 ;
            }
        }
        res.addExt(tuoGuanCountDown);
        res.addExt(isAutoPlay() ? 1 : 0);// 4
	...
	public List<PhzHuCards> buildPhzCards(KlPaohuziTable table,long lookUid) {
		List<PhzHuCards> list = new ArrayList<>();
		for (CardTypeHuxi type : cardTypes) {
			if (type.getAction() == PaohzDisAction.action_zai) {
				// 不是本人并且是栽
	...
	public static void loadWanfaPlayers(Class<? extends Player> cls){
		for (Integer integer:wanfaList){
			PlayerManager.wanfaPlayerTypesPut(integer,cls, KlPaohuziCommandProcessor.getInstance());
		}
	}
